...
if(isset($_POST['update'])){
  ...
  //  $Branch_Phone_Number = $_POST['branch_Phnone_Num'];
    ...
    $update = "UPDATE restaurant_menu SET meal_Name='$Meal_Name',meal_Price=' $Meal_Price',branch_code='$Branch_Code' WHERE meal_id ='$ID'";
    ...
    if($query){
        ...
        echo '</script>';
        echo 'window.location.href = "View_Food_Details.php";';
    }else{
        echo 'Data not saved';
...
if(isset($_POST['delete'])){
  ...
  //  $Branch_Phone_Number = $_POST['branch_Phnone_Num'];
    ...
    $delete = "DELETE restaurant_menu WHERE ID = '$ID'";
...
if(mysqli_num_rows($result)>0){
    while($row = mysqli_fetch_assoc($result)){
        ...
        echo '<td>';
       echo '<button onclick="openUpdateModal('.$row['meal_id'].',\''.$row['meal_Name'] . '\', \'' . $row['meal_Price'] . '\',\'' . $row['branch_code'] . '\')" class="btn btn-warning btn-sm">Update</button>';
         
   //     echo '<button onclick="openUpdateModal('.$row['meal_id'].')" class="btn btn-warning btn-sm">Update</button>';
     

        echo '<form method="post" style="display:inline;">';
        echo '<input type="hidden" name="id" value="'.$row['meal_id'].'">';
       
        echo '</form>'; 
        echo '&nbsp;&nbsp;&nbsp;';

        echo '<button onclick="openDeleteModal('.$row['meal_id'].')" class="btn btn-danger btn-sm">Delete</button>';
        echo '<form method="post" style="display:inline;">';
        echo '<input type="hidden" name="id1" value="'.$row['meal_id'].'">';
...
<div class="w3-container">
    <form id="updateForm" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" name="meal_Name" id="modalMealName" class="form-control" required>
        </div>

        <div class="form-group">
                        <label class="control-label">&nbsp; Meal Price:</label>
                        <input type="text" name="meal_Price" id="modalMealPrice" class="form-control" required>
                    </div>
  ...
  <div class="w3-modal-content w3-animate-top w3-card-4">
    ...
    <div class="w3-container">
      ...
      <div style="text-align: center;">
        <button onclick="confirmDelete()" class="w3-button w3-green" name="delete">Yes</button>
...
//Modal (UPDATE)
function openUpdateModal(id, mealName, mealPrice,branchCode) {
...
function confirmDelete() {
    if (window.deleteRecordId !== undefined) {
        window.location.href = 'View_Food_Details.php?delete=' + window.deleteRecordId;
    }
}
